...
test('createGitHubRelease should be exported', () => {
    assert.ok(typeof lib.createGitHubRelease === 'function', 'createGitHubRelease should be exported');
});

test('createGitHubRelease should return dry-run result without token', async () => {
    const mockFetch = async () => ({ ok: true, json: async () => ({ id: 1, tag_name: 'v1.0.0' }) });

    const result = await lib.createGitHubRelease('owner/repo', { tag_name: 'v1.0.0', name: 'Release 1.0.0', body: 'Test' }, {
        fetchImpl: mockFetch,
        dryRun: true
    });
    ...
    assert.strictEqual(result.release.tagName, 'v1.0.0');
});

test('createGitHubRelease should reject invalid repo format', async () => {
    const result = await lib.createGitHubRelease('invalid', { tag_name: 'v1.0.0' }, {
        fetchImpl: async () => ({ ok: true })
    });

    assert.strictEqual(result.success, false);
    assert.ok(result.error.includes('Invalid repo format'));
});

test('createGitHubRelease should reject missing release data', async () => {
    const result = await lib.createGitHubRelease('owner/repo', null, {
        fetchImpl: async () => ({ ok: true })
    });

    assert.strictEqual(result.success, false);
    assert.ok(result.error.includes('releaseData with tag_name'));
});

test('createGitHubRelease should check for existing release when update is false', async () => {
    ...
    const mockFetch = async (url, options) => {
        if (url.includes('/tags/')) {
            ...
            return {
                ok: true,
                json: async () => ({ id: 123, tag_name: 'v1.0.0' })
            };
        }
        return { ok: false, status: 404 };
    };

    const result = await lib.createGitHubRelease('owner/repo', { tag_name: 'v1.0.0', name: 'Release 1.0.0' }, {
        fetchImpl: mockFetch,
        token: 'fake-token',
        update: false
    });
    ...
    assert.ok(result.existingRelease);
});

test('createGitHubRelease should update existing release when update is true', async () => {
    const mockFetch = async (url, options) => {
        if (url.includes('/tags/')) {
            return {
                ok: true,
                json: async () => ({ id: 123, tag_name: 'v1.0.0', upload_url: 'https://uploads.github.com/{?name}' })
            };
        }
        if (options && options.method === 'PATCH') {
            return {
                ...
                json: async () => ({
                    ...
                    upload_url: 'https://uploads.github.com/{?name}'
                })
            };
        }
        return { ok: false, status: 404 };
    ...
    const result = await lib.createGitHubRelease('owner/repo', { tag_name: 'v1.0.0', name: 'Updated Release' }, {
        fetchImpl: mockFetch,
        token: 'fake-token',
        update: true
    });
    ...
    assert.strictEqual(result.release.id, 123);
});

test('createGitHubRelease should create new release when no existing release', async () => {
    const mockFetch = async (url, options) => {
        if (url.includes('/tags/')) {
            return { ok: false, status: 404 };
        }
    ...
    assert.strictEqual(result.success, true);
    assert.strictEqual(result.action, 'create');
    assert.strictEqual(result.release.id, 456);
});

test('uploadReleaseAsset should be exported', () => {
    assert.ok(typeof lib.uploadReleaseAsset === 'function', 'uploadReleaseAsset should be exported');
});

test('uploadReleaseAsset should return dry-run result without token', async () => {
    const result = await lib.uploadReleaseAsset(
        { id: 123, uploadUrl: 'https://uploads.github.com/{?name,label}' },
        { name: 'app.zip', path: '/non/existent/file.zip' },
        { dryRun: true }
    );
...
test('uploadReleaseAsset should reject missing release info', async () => {
    const result = await lib.uploadReleaseAsset(null, { name: 'app.zip', path: '/non/existent.zip' });

    assert.strictEqual(result.success, false);
    assert.ok(result.error.includes('release and fetchImpl'));
});

test('uploadReleaseAsset should reject missing file', async () => {
    const result = await lib.uploadReleaseAsset(
        { id: 123, uploadUrl: 'https://uploads.github.com/{?name}' },
        { name: 'app.zip', path: '/non/existent/file.zip' },
        { fetchImpl: async () => ({}) }
    );
...
test('deleteExistingAsset should be exported', () => {
    assert.ok(typeof lib.deleteExistingAsset === 'function', 'deleteExistingAsset should be exported');
});

test('deleteExistingAsset should return dry-run result without token', async () => {
    const result = await lib.deleteExistingAsset('https://api.github.com/assets/123', { dryRun: true });

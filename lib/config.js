...
const CONFIG_SCHEMA = {
    ...
    plugins: {
        ...
        fields: {
            ...
            allowedHosts: { type: 'array', desc: '允许的插件远程源 hosts 列表' }
        }
    }
};
            ...
            for (const [subKey, subValue] of Object.entries(value)) {
                ...
                if (subSchema && subSchema.type === 'array') {
                    if (!Array.isArray(subValue)) {
                        errors.push(`"${key}.${subKey}" 应为 array 类型`);
                    } else if (subValue.some(item => typeof item !== 'string')) {
                        errors.push(`"${key}.${subKey}" 数组中的每一项都应为 string 类型`);
                    }
                    continue;
                }

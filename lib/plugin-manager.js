...
function stripPluginSignatureFields(plugin) {
    if (!plugin || typeof plugin !== 'object' || Array.isArray(plugin)) {
        return plugin;
    }

    const cloned = JSON.parse(JSON.stringify(plugin));
    delete cloned.signature;
    delete cloned.publicKey;
    delete cloned.keyId;
    delete cloned.algorithm;
    delete cloned.signatureVerified;
    return cloned;
}

function calculatePluginSigningChecksum(plugin) {
    return calculateSha256(JSON.stringify(stripPluginSignatureFields(plugin), null, 2));
}

function normalizePluginInput(pluginInput) {
    if (typeof pluginInput === 'string') {
        const content = pluginInput.trim();
        if (!content) {
        ...
        try {
            ...
            return { valid: false, error: 'JSON 解析失败', parseError: e.message };
        }
    }

    if (!pluginInput || typeof pluginInput !== 'object' || Array.isArray(pluginInput)) {
        return { valid: false, error: '插件必须是 JSON 对象' };
    }

    return {
        valid: true,
        plugin: JSON.parse(JSON.stringify(pluginInput)),
        content: JSON.stringify(pluginInput, null, 2),
        rawContent: JSON.stringify(pluginInput)
    };
}

function validatePluginInput(pluginInput, options = {}) {
    try {
        const normalized = normalizePluginInput(pluginInput);
        if (!normalized.valid) {
            return {
                valid: false,
                error: normalized.error,
                parseError: normalized.parseError || null,
                plugin: null
            };
        }
        ...
        if (!validation.valid) {
            return { valid: false, error: validation.error, plugin: null };
        }

        const checksum = calculateSha256(normalized.content);
        const signingChecksum = calculatePluginSigningChecksum(plugin);
        const fileSize = Buffer.byteLength(normalized.content, 'utf8');
        const result = {
            valid: true,
        ...
        if (options.verifySignature) {
            const signatureInfo = verifyPluginSignature(signingChecksum, {
                signature: options.signature || plugin.signature,
                publicKey: options.publicKeyPem || options.publicKey || plugin.publicKey,
                keyId: options.keyId || plugin.keyId,
                algorithm: options.algorithm || plugin.algorithm
            }, {
        ...
        return { valid: false, error: e.message, path: options.path || null, plugin: null };
    }
}

function validatePluginJson(pluginPath, options = {}) {
    try {
        if (!fs.existsSync(pluginPath)) {
            return { valid: false, error: '文件不存在', path: pluginPath };
        }

        const content = fs.readFileSync(pluginPath, 'utf8');
        return validatePluginInput(content, {
            ...options,
            path: pluginPath
        });
    } catch (e) {
        return { valid: false, error: e.message, path: pluginPath, plugin: null };
    }
}

function generateIndexEntry(plugin, options = {}) {
    const entry = {
        name: plugin.name,
    ...
    if (options.checksum) {
        entry.checksum = options.checksum;
    }
    ...
    if (options.publicKey) {
        entry.publicKey = options.publicKey;
    }

    if (options.keyId) {
        entry.keyId = options.keyId;
    }

    if (options.algorithm) {
        entry.algorithm = options.algorithm;
    }

    if (options.additional) {
        for (const key of ['homepage', 'license', 'keywords']) {
            if (options.additional[key]) {
                entry[key] = options.additional[key];
            }
        }
    }
...
function generatePluginMetadata(plugin, options = {}) {
    return {
        name: plugin.name,
        version: plugin.version,
        ...
        sha256: options.sha256 || options.checksum || null,
        indexEntry: options.indexEntry || null,
...
function generateSubmissionChecklist(plugin, options = {}) {
    const checks = [];
    const now = new Date().toISOString();
    
    checks.push({ label: '插件 JSON 格式有效', checked: options.validPlugin !== false, required: true });
    checks.push({ label: 'version 符合语义化版本 (x.y.z)', checked: plugin.version && /^\d+\.\d+\.\d+$/.test(plugin.version), required: true });
    checks.push({ label: '包含 name 和 data 字段', checked: plugin.name && plugin.data, required: true });
    checks.push({ label: 'data 字段为有效对象', checked: plugin.data && typeof plugin.data === 'object', required: true });
    checks.push({ label: '提供 description 描述', checked: plugin.description && typeof plugin.description === 'string', required: false });
    checks.push({ label: '提供 author 作者信息', checked: plugin.author && typeof plugin.author === 'string', required: false });
    checks.push({ label: 'SHA-256 校验和已计算', checked: !!options.checksum, required: true });
    checks.push({ label: 'sourceUrl 已设置', checked: !!options.sourceUrl, required: false });
    checks.push({ label: 'github 引用已设置', checked: !!options.github, required: false });
    checks.push({ label: '已生成索引条目', checked: !!options.indexEntry, required: true });
    checks.push({ label: 'Ed25519 签名已生成 (可选)', checked: !!options.signature, required: false });
    checks.push({ label: '公钥已提供 (签名用)', checked: !!options.publicKey, required: false });
    checks.push({ label: 'keyId 已设置 (签名用)', checked: !!options.keyId, required: false });
    checks.push({ label: '文件已测试加载', checked: options.loadTested === true, required: true });
    checks.push({ label: '本地安装测试通过', checked: options.localInstallTested === true, required: false });

    const passed = checks.filter(c => c.required).every(c => c.checked);
    const totalRequired = checks.filter(c => c.required).length;
    const checkedRequired = checks.filter(c => c.required && c.checked).length;
    ...
    return {
        ...
        passed,
        summary: `${checkedRequired}/${totalRequired} 必需项通过`,
        checks
    };
}

function normalizeReleaseKitOutputDir(pluginPath, options = {}) {
    const outputDir = options.outputDir || path.join(path.dirname(pluginPath), 'dist');
    fs.mkdirSync(outputDir, { recursive: true });
    return outputDir;
}

function normalizeGithubMetadata(options = {}) {
    let github = options.github || null;
    if (options.github) {
        const parsed = parseGitHubShorthand(options.github);
        if (parsed.valid) {
            github = {
                owner: parsed.owner,
                repo: parsed.repo,
...
function buildPackArtifacts(validation, options = {}) {
    const plugin = validation.plugin;
    const checksum = validation.checksum;
    const fileSize = validation.fileSize || (validation.path && fs.existsSync(validation.path)
        ? fs.statSync(validation.path).size
        : Buffer.byteLength(validation.content || '', 'utf8'));
    const filePath = validation.path || null;
    const sourceUrl = options.sourceUrl || null;
    const github = normalizeGithubMetadata(options);
    ...
    const summary = {
        ...
        fileSize,
        filePath,
        languages: plugin.data ? Object.keys(plugin.data) : [],
        styles: plugin.styles ? Object.keys(plugin.styles) : []
    };

    let signatureInfo = null;
    let finalIndexEntry = generateIndexEntry(plugin, {
        checksum,
        sourceUrl,
        github: github && github.spec ? github.spec : github
    });

    if (options.signPrivateKey) {
        const signResult = signWithEd25519(validation.signingChecksum, options.signPrivateKey);
        if (!signResult.success) {
            return { success: false, error: signResult.error };
        }

        signatureInfo = buildSignatureInfo(
        ...
        finalIndexEntry = generateIndexEntry(plugin, {
            checksum,
            sourceUrl,
            github: github && github.spec ? github.spec : github,
    ...
    return {
        ...
        github,
        summary,
        indexEntry: finalIndexEntry,
        signature: signatureInfo
    };
}

function packPluginData(pluginInput, options = {}) {
    try {
        const validation = validatePluginInput(pluginInput);
        if (!validation.valid) {
            return { success: false, error: validation.error };
        }

        const artifactResult = buildPackArtifacts(validation, options);
        if (!artifactResult.success) {
            return artifactResult;
        }

        let outputMetadataPath = null;
        if (options.outputMetadata) {
            const metadataContent = generatePluginMetadata(artifactResult.plugin, {
                checksum: artifactResult.checksum,
                sha256: artifactResult.checksum,
                sourceUrl: artifactResult.sourceUrl,
                github: artifactResult.github,
                fileSize: artifactResult.fileSize,
                createdAt: new Date().toISOString(),
                indexEntry: artifactResult.indexEntry,
                signature: artifactResult.signature ? artifactResult.signature.signature : null,
                publicKey: artifactResult.signature ? artifactResult.signature.publicKey : null,
                keyId: artifactResult.signature ? artifactResult.signature.keyId : null,
                algorithm: artifactResult.signature ? artifactResult.signature.algorithm : null
            });

            fs.writeFileSync(options.outputMetadata, JSON.stringify(metadataContent, null, 2), 'utf8');
            outputMetadataPath = options.outputMetadata;
        }
        ...
        return {
            ...
            signature: artifactResult.signature
        };
    } catch (e) {
...
function generateReleaseKitData(pluginInput, options = {}) {
    try {
        const validation = validatePluginInput(pluginInput);
        if (!validation.valid) {
            return { success: false, error: validation.error };
        ...
        const plugin = artifactResult.plugin;
        ...
        const checklist = generateSubmissionChecklist(plugin, {
            validPlugin: true,
            checksum: artifactResult.checksum,
            sourceUrl: artifactResult.sourceUrl,
            github: artifactResult.github && artifactResult.github.spec,
            indexEntry: artifactResult.indexEntry,
            signature: artifactResult.signature ? artifactResult.signature.signature : null,
            publicKey: artifactResult.signature ? artifactResult.signature.publicKey : null,
            keyId: artifactResult.signature ? artifactResult.signature.keyId : null,
            loadTested: options.loadTested !== false,
            localInstallTested: options.skipLocalInstallTest ? false : (options.localInstallTested || false)
        });

        const submissionMarkdown = generateSubmissionMarkdown(plugin, {
            checksum: artifactResult.checksum,
            sourceUrl: artifactResult.sourceUrl,
            github: artifactResult.github && artifactResult.github.spec,
...
function generateSubmissionMarkdown(plugin, options = {}) {
    ...
    const lines = [];
    lines.push(`# Plugin Submission: ${plugin.name}`);
    lines.push('');
    ...
    if (!sourceUrl && !github) {
        lines.push(`- _No sourceUrl or github specified_`);
    }
    lines.push('');
    lines.push(`## Checksum`);
    lines.push('');
    ...
    if (signature && publicKey) {
        ...
        lines.push('');
        lines.push(`- **algorithm**: ${algorithm || 'Ed25519'}`);
        if (keyId) {
        ...
        lines.push('```json');
        lines.push(`"signature": "${signature}"`);
        lines.push(`"publicKey": "${publicKey.replace(/\n/g, '\\n')}"`);
        if (keyId) {
            lines.push(`"keyId": "${keyId}"`);
        }
        lines.push(`"algorithm": "${algorithm || 'Ed25519'}"`);
    ...
    if (indexEntry) {
        ...
        lines.push('');
        lines.push(`可直接复制到插件索引仓库的 \`index.json\`:`);
        lines.push('');
        lines.push('```json');
        lines.push(JSON.stringify(indexEntry, null, 2));
        lines.push('```');
        lines.push('');
    }
    ...
    if (checklist) {
        ...
        lines.push('');
        lines.push(`检查时间: ${checklist.date}`);
        lines.push(`状态: ${checklist.passed ? '✅ 通过' : '❌ 未通过'}`);
        lines.push('');
        ...
        for (const check of checklist.checks) {
            ...
            lines.push(`| ${check.label} | ${status} | ${required} |`);
        }
        lines.push('');
    }

    lines.push('---\n');
    lines.push('_* Generated by git-sao-hua plugin release-kit *_');

    return lines.join('\n');
}

function generateReleaseKit(pluginPath, options = {}) {
    try {
        const validation = validatePluginJson(pluginPath);
        if (!validation.valid) {
            return { success: false, error: validation.error };
        }

        const plugin = validation.plugin;
        const artifactResult = generateReleaseKitData(plugin, {
            ...options,
            loadTested: true,
            localInstallTested: options.skipLocalInstallTest ? false : (options.localInstallTested || false)
        });
        ...
        const outputDir = normalizeReleaseKitOutputDir(filePath, options);
        const outputBasename = options.name || plugin.name;
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');

        const metadataPath = path.join(outputDir, `${outputBasename}-release-${timestamp}.metadata.json`);
        const indexEntryPath = path.join(outputDir, `${outputBasename}-release-${timestamp}.index-entry.json`);
        const submissionMdPath = path.join(outputDir, `${outputBasename}-release-${timestamp}.submission.md`);

        fs.writeFileSync(metadataPath, JSON.stringify(artifactResult.metadata, null, 2), 'utf8');
        fs.writeFileSync(indexEntryPath, JSON.stringify(artifactResult.indexEntry, null, 2), 'utf8');
        fs.writeFileSync(submissionMdPath, artifactResult.submissionMarkdown, 'utf8');
        ...
        return {
            ...
            indexEntry: artifactResult.indexEntry,
            checklist: artifactResult.checklist,
            submissionMarkdown: artifactResult.submissionMarkdown,
            signature: artifactResult.signature
        };
    } catch (e) {
        return { success: false, error: e.message };
...
function parsePemPrivateKey(pemKey) {
    try {
        const privateKey = crypto.createPrivateKey(pemKey);
        return { success: true, key: privateKey };
    } catch (e) {
        return { success: false, error: 'PEM 私钥解析失败: ' + e.message };
    }
}
...
function parsePemPublicKey(pemKey) {
    try {
        ...
        return { success: true, key: publicKey };
    } catch (e) {
        return { success: false, error: 'PEM 公钥解析失败: ' + e.message };
    }
}

function signWithEd25519(content, privateKeyPem) {
    try {
        const parseResult = parsePemPrivateKey(privateKeyPem);
        if (!parseResult.success) {
            return { success: false, error: parseResult.error };
        }

        const signature = crypto.sign(null, Buffer.from(content, 'utf8'), parseResult.key);
        return {
            success: true,
        ...
        return { success: false, error: '签名失败: ' + e.message };
    }
}

function verifyEd25519Signature(content, signatureB64, publicKeyPem) {
    try {
        const parseResult = parsePemPublicKey(publicKeyPem);
        if (!parseResult.success) {
            return { success: false, error: parseResult.error };
        }

        const signature = Buffer.from(signatureB64, 'base64');
        const isValid = crypto.verify(null, Buffer.from(content, 'utf8'), parseResult.key, signature);

        return {
            success: true,
            valid: isValid,
            error: isValid ? null : '签名无效'
        };
    } catch (e) {
        return { success: false, error: '验签失败: ' + e.message };
    }
}

function buildSignatureInfo(signature, publicKey, keyId, algorithm) {
    return {
        signature: signature || null,
        publicKey: publicKey || null,
        keyId: keyId || null,
        algorithm: algorithm || ED25519_ALGORITHM
    };
}

function verifyPluginSignature(content, signatureInfo = {}, options = {}) {
    const normalized = buildSignatureInfo(
    ...
    if (!normalized.signature && !normalized.publicKey) {
    ...
    if (!normalized.signature || !normalized.publicKey) {
        return {
            ...
            verified: false,
            ...normalized,
            error: '签名信息不完整，需要同时提供 signature 和 publicKey'
        };
    }

    if ((normalized.algorithm || ED25519_ALGORITHM) !== ED25519_ALGORITHM) {
        return {
            valid: false,
            verified: false,
            ...normalized,
            error: `暂不支持的签名算法: ${normalized.algorithm}`
        };
    }

    const verifyResult = verifyEd25519Signature(content, normalized.signature, normalized.publicKey);
    if (!verifyResult.success || !verifyResult.valid) {
        return {
            valid: false,
            verified: false,
            ...normalized,
            error: verifyResult.error || '签名无效'
        };
    }
...
function loadPemFile(filePath) {
    try {
        if (!fs.existsSync(filePath)) {
            return { success: false, error: '文件不存在' };
        }
        const content = fs.readFileSync(filePath, 'utf8');
        return { success: true, pem: content };
    } catch (e) {
        return { success: false, error: e.message };
    }
}
...
function buildPluginMetadata(plugin, metadata = {}) {
    return {
        ...
        signature: metadata.signature || plugin.signature || null,
        publicKey: metadata.publicKey || plugin.publicKey || null,
        keyId: metadata.keyId || plugin.keyId || null,
        algorithm: metadata.algorithm || plugin.algorithm || null,
        signatureVerified: metadata.signatureVerified ?? plugin.signatureVerified ?? null
    };
}
...
function extractPluginSummary(plugin, pluginPath, lockEntry = null) {
    return {
        ...
        signature: plugin.signature || (lockEntry && lockEntry.signature) || null,
        publicKey: plugin.publicKey || (lockEntry && lockEntry.publicKey) || null,
        keyId: plugin.keyId || (lockEntry && lockEntry.keyId) || null,
        algorithm: plugin.algorithm || (lockEntry && lockEntry.algorithm) || null,
        signatureVerified: plugin.signatureVerified || (lockEntry && lockEntry.signatureVerified) || null
    };
}
...
function packPlugin(pluginPath, options = {}) {
    try {
        const validation = validatePluginJson(pluginPath);
        if (!validation.valid) {
            return { success: false, error: validation.error };
        }
        return packPluginData(validation.plugin, options);
    } catch (e) {
        return { success: false, error: e.message };
    }
}
...
async function installPluginFromUrl(pluginUrl, pluginsDir, options = {}) {
    try {
        ...
        const signingChecksum = calculatePluginSigningChecksum(fetchResult.plugin);

        const plugin = fetchResult.plugin;
        const signatureInfo = verifyPluginSignature(signingChecksum, {
            ...
            requireSignature: options.verifySignature === true
        });

        if (!signatureInfo.valid) {
            return { success: false, error: signatureInfo.error };
        }
        ...
        const installResult = installPluginObject(fetchResult.plugin, pluginsDir, {
            ...
            publicKey: plugin.publicKey || null,
            keyId: plugin.keyId || null,
            algorithm: plugin.algorithm || null,
            signatureVerified: signatureInfo.verified
        });

        if (installResult.success) {
            installResult.signatureInfo = signatureInfo;
        }
...
async function installPluginFromIndex(pluginName, indexUrl, pluginsDir, options = {}) {
    try {
        ...
        const computedChecksum = plugin.checksum
            ? plugin.checksum
            : calculateSha256(fetchResult.rawContent);
        const signingChecksum = calculatePluginSigningChecksum(fetchResult.plugin);

        const remotePlugin = fetchResult.plugin;
        const signatureInfo = verifyPluginSignature(signingChecksum, {
            signature: plugin.signature || remotePlugin.signature,
            publicKey: plugin.publicKey || remotePlugin.publicKey,
            keyId: plugin.keyId || remotePlugin.keyId,
            algorithm: plugin.algorithm || remotePlugin.algorithm
        }, {
            requireSignature: options.verifySignature === true
        });
        ...
        const installResult = installPluginObject(fetchResult.plugin, pluginsDir, metadata);

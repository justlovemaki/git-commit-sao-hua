...
function validatePluginJson(pluginPath, options = {}) {
    try {
        ...
        try {
            ...
            return { valid: false, error: 'JSON 解析失败', path: pluginPath, parseError: e.message };
        }

        const validation = validatePlugin(plugin);
        if (!validation.valid) {
            return { valid: false, error: validation.error, path: pluginPath, plugin: null };
        }
...
function generateReleaseKit(pluginPath, options = {}) {
    try {
        ...
        let homepage = options.homepage || null;
        ...
        let indexEntry = generateIndexEntry(plugin, {
            checksum,
            sourceUrl,
        ...
        if (options.signPrivateKey) {
            ...
            indexEntry = generateIndexEntry(plugin, {
                checksum,
                sourceUrl,
                github: github && github.spec ? github.spec : github,
        ...
        const metadata = generatePluginMetadata(plugin, {
            ...
            publicKey: signatureInfo ? signatureInfo.publicKey : null,
            keyId: signatureInfo ? signatureInfo.keyId : null,
            algorithm: signatureInfo ? signatureInfo.algorithm : null
        });

        fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2), 'utf8');
        fs.writeFileSync(indexEntryPath, JSON.stringify(indexEntry, null, 2), 'utf8');

        const checklist = generateSubmissionChecklist(plugin, {
            validPlugin: true,
            checksum,
            sourceUrl,
            github: github && github.spec,
            indexEntry,
            signature: signatureInfo ? signatureInfo.signature : null,
            publicKey: publicKey,
            keyId: signatureInfo ? signatureInfo.keyId : null,
        ...
        fs.writeFileSync(submissionMdPath, submissionMarkdown, 'utf8');
...
function packPlugin(pluginPath, options = {}) {
    try {
        ...
        if (options.outputMetadata) {
            const metadataContent = generatePluginMetadata(plugin, {
                checksum,
                sha256: checksum,
                sourceUrl,
                github,

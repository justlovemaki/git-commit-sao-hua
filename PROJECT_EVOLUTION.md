...
| **npm 发布** | ✅ 准备就绪 | lib/ 和 cli/ 均已配置 publish 脚本，待执行 |
...
3. **npm 待发布** — 配置已就绪，需执行 npm publish
...
- ~~**npm 发布**：将 lib/ (git-sao-hua-core) 和 cli/ (git-sao-hua) 发布到 npm~~ ✅ 配置就绪
- ~~**CLI 集成智能检测**：将 VSCode 的检测逻辑移植到 lib/，CLI 支持 `--auto` 自动检测~~ ✅ 已完成
- ~~**GitHub Action CI**：添加 CI 工作流，自动运行测试~~ ✅ 已完成

### 下一步（待执行）
- **执行 npm publish**：推送 git-sao-hua-core 和 git-sao-hua 到 npm registry
...
| 轮次 | 日期 | 类型 | 改动概要 | 阶段变化 |
|------|------|------|---------|---------|
| 最新 | 2026-03-26 | 🚀 大演进 | 智能检测共享 + npm 发布准备 v1.20.0 — CLI 支持 --auto 智能检测 + CI/CD 工作流 | 不变（Stage 3 内功能完善） |
| -1 | 2026-03-25 | 🚀 大演进 | 核心库抽取 v1.19.0 — 三端共享 lib/ + 29 测试用例 | 不变（Stage 3 内架构跃迁） |
| -2 | 2026-03-25 | 🚀 大演进 | CLI 命令行工具 v1.18.0 — 突破 VSCode 限制 | Stage 2→3 |
| -3 | 2026-03-23 | 🔧 中迭代 | 代码模式智能识别 v1.17.0 | 不变 |
| -4 | 2026-03-22 | 🔧 中迭代 | 智能检测日志分析 v1.16.0 | 不变 |

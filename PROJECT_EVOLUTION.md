...
- **改进内容**: 代码模式智能识别功能
  ...
  - 新增路由变更模式检测：React Router、Vue Router、Angular Router、Next.js 路由等路由配置相关模式
  - 新增组件/模板模式检测：React 组件、Vue 组件、Web 组件、模板引擎等 UI 组件相关模式
  - 新增样式布局模式检测：CSS 布局、Flexbox、Grid、Sass/Less 样式等样式调整相关模式
  - 基于 AST 分析和 Diff 关键词双重检测机制，提高模式识别准确率
  - 模式识别优先级高于文件类型分析，低于 AST 代码结构分析
  - 检测到特定代码模式时，对应 Commit 类型置信度自动提升一级
  - 版本号同步：package.json 和 extension.js 更新到 1.17.0
  - 文档同步更新：README.md 补充 v1.17.0 新功能说明
- **风险等级**: 低风险（仅新增代码模式识别逻辑，无核心逻辑变更）
- **收益**: 进一步提升智能检测准确率，覆盖更多实际开发场景，让 Commit 类型推荐更加精准和智能化
- ✅ **数据库操作模式检测** - SQL/ORM/MongoDB/Redis 等数据层变更识别
- ✅ **API/HTTP 请求模式检测** - fetch/axios/GraphQL/REST API 等接口变更识别
- ✅ **路由变更模式检测** - React/Vue/Angular/Next.js 路由配置变更识别
- ✅ **组件/模板模式检测** - React/Vue/Web 组件和模板引擎变更识别
- ✅ **样式布局模式检测** - CSS/Flexbox/Grid/Sass/Less 样式调整识别
- ✅ **双重检测机制** - AST 分析 + Diff 关键词双重检测，提高准确率
- ✅ **优先级优化** - 模式识别优先级：AST代码结构 > 代码模式 > 文件类型
- ✅ **置信度提升机制** - 检测到特定模式时自动提升对应类型置信度等级
- ✅ **版本号同步** - package.json 和 extension.js 更新到 1.17.0
- ✅ **文档同步更新** - README.md 补充 v1.17.0 新功能说明和使用指南
...
- **改进内容**: 快捷设置面板功能
  - 新增 `gitCommitSaoHua.quickSettings` 命令，打开快捷设置面板
  - 使用 QuickPick 提供直观的 UI 调节智能检测相关设置
  - 支持调节高置信度阈值（范围 1-10，默认 3）
  - 支持调节中等置信度阈值（范围 1-5，默认 1）
  - 支持切换 AST 分析优先开关（默认启用）
  - 支持切换 Diff 分析优先开关（默认启用）
  - 支持切换描述提示开关（默认启用）
  - 数值设置带输入验证，确保在有效范围内
  - 开关设置提供启用/禁用选项
  - 调节后即时保存到工作区配置，并显示确认消息
  - 版本号同步：package.json 和 extension.js 更新到 1.14.0
  - 文档同步更新：README.md 补充快捷设置面板功能说明
- **风险等级**: 低风险（仅新增设置入口，无核心逻辑变更）
- **收益**: 提升用户配置体验，无需打开 VSCode 设置界面即可快速调整智能检测参数，让配置更直观、更便捷
- ✅ **快捷设置面板命令** - 新增 `gitCommitSaoHua.quickSettings` 命令
- ✅ **高置信度阈值 UI 调节** - 在 QuickPick 中直接输入新值（范围 1-10）
- ✅ **中等置信度阈值 UI 调节** - 在 QuickPick 中直接输入新值（范围 1-5）
- ✅ **AST 优先级开关 UI 调节** - 在启用/禁用之间快速切换
- ✅ **Diff 优先级开关 UI 调节** - 在启用/禁用之间快速切换
- ✅ **描述提示开关 UI 调节** - 在启用/禁用之间快速切换
- ✅ **输入验证** - 数值设置自动验证范围，超出范围时提示错误
- ✅ **即时保存** - 调节后即时保存到工作区配置（ConfigurationTarget.Workspace）
- ✅ **确认消息** - 保存后显示确认消息，告知用户设置已成功更新
- ✅ **文档同步更新** - README.md 补充快捷设置面板功能说明和配置项文档

### v1.13.0 - Commit 描述智能生成 📝
...
- ✅ quickSettings 快捷设置面板（v1.14.0）
- ✅ 高置信度阈值 UI 调节（v1.14.0）
- ✅ 中等置信度阈值 UI 调节（v1.14.0）
...
- ✅ 多来源一致置信度提升（v1.15.0）
- ✅ 智能检测日志记录（v1.16.0）
- ✅ 用户反馈收集（采纳/跳过/修改）（v1.16.0）
- ✅ 检测日志查看命令（v1.16.0）
- ✅ 检测日志清空命令（v1.16.0）
- ✅ 准确率分析命令（v1.16.0）
- ✅ enableDetectionLogging 配置项（v1.16.0）
- ✅ 代码模式识别（v1.17.0）
- ✅ 数据库操作模式检测（v1.17.0）
- ✅ API/HTTP 请求模式检测（v1.17.0）
- ✅ 路由变更模式检测（v1.17.0）
- ✅ 组件/模板模式检测（v1.17.0）
- ✅ 样式布局模式检测（v1.17.0）
